---
title: "Predicting Airplane Departure Delays at MSP"
...
Using LASSO allowed us to simplify the base least squares model, and determine which predictors in the dataset were not significant in estimating departure delay. As seen in the final model, we only have 10 remaining predictors out of 20. However, we did not stop here. When using LASSO, we can further simplify our model by choosing a different tuning parameter in the model. This choice does not make our predictions significantly worse, and is a better choice for our model (reference lambda plot). This parsimonious model also had the lowest MAE (12.2363), which means on average, our model prediction of departure delay has a mean average error of  about 12 minutes, the best of all models we fit. 
...
The distribution of prediction errors is roughly bell-shaped and centered near zero, with a slight tendency toward underpredicting delays (more large positive errors than large negative errors). Overall, these diagnostics suggest the model's structure is appropriate for this prediction task.
...
The predicted versus actual delay plot shows that most predictions fall in the 2-7 minute range. The model consistently predicts small delays, which works well for flights with minor delays, but struggles to accurately anticipate severe delays accurately. This conservative prediction pattern is understandable given that most flights experience minor delays, and the model learns from this historical pattern. Also, our model does not predict any departures that are negative, or that depart early. 
...
Second, the model's tendency to predict moderate delays for most situations means it will systematically underestimate severe delays. This could disproportionately impact routes or times of day that experience more variable delay patterns. If certain routes consistently face longer delays, passengers and operations staff on those routes would receive less accurate predictions.

Third, there are equity considerations. If the model performs differently for different carriers, routes, or customer segments, this could worsen or exacerbate existing service quality disparities. For instance, if budget routes or flights serving smaller communities receive less accurate predictions, those passengers would have less reliable information for planning their travel.
...
There was work done by all group members on models/visualization. Most of the work done by Adam and Olivia did not get included in the report (model building and analysis that was not LASSO, and thus did not get used). Most of the work done in building the LASSO model we ended up choosing was done by William. He was thus responsible for data wrangling, which was redone midway through the project when we shifted our approach from using only positive delays to including all departure delays in our analysis. He also revisited which predictors to drop and reconsider our variable selection strategy.The visualizations were split fairly evenly among group members. The visualizations in the ‘data’ section were done by Liam, the visualizations pertaining to LASSO/Lambda were done by Olivia, and the visualizations done in the model evaluations section were done by Adam.
The sections were also split up.  William wrote the implementation section, Olivia wrote the model building section, and Adam did the data and model evaluation sections. Liam also took good initiative in setting up and encouraging group collaboration through the use of Github, which was beneficial to the whole group. 
...
### obtaining lasso coefs and plotting them to see convergence
...
lasso_coefs %>%
  ...
  geom_line()  +
  geom_text(data = lasso_coefs %>% filter(lambda>0.5), aes(x = 0.001, label = term), size = 2)   + 
  scale_x_log10(limits = c(-2, 2)) +  
  guides(color = FALSE) + labs(title = "Figure 2")
```
...
### Data wrangling/extra visualizations 
