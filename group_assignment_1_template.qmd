...
William took the lead on building and implementing three regression models and picking the most appropiate one: LASSO, KNN, and least squares. He was responsible for data wrangling, which he had to redo midway through the project when we decided to shift our approach from using only positive delays to including all departure delays in our analysis. He also revisited which predictors to drop and reconsider our variable selection strategy, as our initial approach had kept all predictors without consideration of their relevance. William wrote the implementation section and also created the initial exploratory visualization that we are using to understand the data structure and guided our modeling decisions, including identifying patterns in delays across carriers.
