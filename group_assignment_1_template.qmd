...
```{r}
#mutate(delta=as.numeric(carrier=="DL"), skywest=as.numeric(carrier=="OO")) %>% 
...
knn_recipe <- recipe(dep_delay ~ ., data = flight_train_dest_delayed) %>%
  step_rm(tailnum, plane_engine, plane_model, plane_manufacturer, plane_seats,plane_type,dest_dst, dest_tz) %>% # Remove tailnum - too many unique values
